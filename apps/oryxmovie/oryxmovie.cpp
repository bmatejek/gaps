...
static std::vector<long> surface = std::vector<long>();
static int movie_iteration = 0;
...
ReadMovieData(void)
{
    if (surface.size()) surface = std::vector<long>();

    char movie_filename[4096];
    sprintf(movie_filename, "movies/%s/%06ld-%04ld.pts", prefix, segmentation_index, movie_iteration);

    FILE *fp = fopen(movie_filename, "rb"); 
    if (!fp) return 0;
...
static void DrawSegment(int segment_index)
{
    ...
    glPointSize(2.0);
    glBegin(GL_POINTS);
    for (unsigned int iv = 0; iv < surface.size(); ++iv) {
        long ix, iy, iz;
        IndexToIndices(surface[iv], ix, iy, iz);
        glVertex3f(ix, iy, iz);
    }
    glEnd();
    glPointSize(1.0);
...
void GLUTKeyboard(unsigned char key, int x, int y)
{
    ...
    switch(key) {
        ...
        case 'm': {
            movie_iteration++;
            if (!ReadMovieData()) movie_iteration--;
            break;
        }
        ...
        case 'n': {
            movie_iteration--;
            if (movie_iteration < 0) movie_iteration = 0;
            ReadMovieData();
            break;
        }
