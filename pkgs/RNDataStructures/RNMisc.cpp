...
R3CharGrid *RNReadH5ImageFile(const char *h5_filename, const char *dataset_name) 
{
    try {
        ...
        R3CharGrid *grid = new R3CharGrid(xres, yres, zres);

        // get the class type
        H5T_class_t class_type = dataset.getTypeClass();
        if (class_type == H5T_INTEGER) {
            // read in the dataset
            unsigned char *data = new unsigned char[grid->NEntries()];
            H5::DataSpace mem_space(rank, dims);
            dataset.read(data, H5::PredType::NATIVE_UINT8, mem_space, dataspace);

            unsigned long ii = 0;
            for (int iz = 0; iz < zres; ++iz) {
                for (int iy = 0; iy < yres; ++iy) {
                    for (int ix = 0; ix < xres; ++ix, ++ii) {
                        grid->SetGridValue(ix, iy, iz, data[ii]);
                    }
                }
            }
...
R3CharGrid *RNReadH5ImageFile(const char *h5_filename, const char *dataset_name) 
{
    try {
        ...
        // get the class type
        unsigned char *data = new unsigned char[zres * yres * xres];
        H5::DataSpace mem_space(rank, dims);
        dataset.read(data, H5::PredType::NATIVE_UINT8, mem_space, dataspace);
